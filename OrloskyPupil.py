...
import matplotlib
matplotlib.use('Agg')  # non-GUI backend if we use plt
...
# Crop to a single center rectangle and zoom by scaling it up (e.g. 10x = small center ROI blown up to full size).
def crop_roi_and_zoom(image, zoom=10, width=640, height=480):
    h, w = image.shape[:2]
    roi_w = max(1, width // zoom)
    roi_h = max(1, height // zoom)
    x0 = (w - roi_w) // 2
    y0 = (h - roi_h) // 2
...
def check_contour_pixels(contour, image_shape, debug_mode_on):
    ...
    if len(contour) < 5:
        return [0, 0, None]  # Not enough points to fit an ellipse
...
def process_frames(thresholded_image_strict, thresholded_image_medium, thresholded_image_relaxed, frame, gray_frame, darkest_point, debug_mode_on, render_cv_window):
    ...
    total_pixels = [0, 0, None]  # default so we don't reference undefined later
    ...
    # Create an empty image to draw contours (use last valid image_array index)
    contour_img3 = np.zeros_like(image_array[0])
...
def open_camera(camera_index=0):
    """Open camera in a cross-platform way."""
    # On Windows, CAP_DSHOW often works better; on macOS/Linux use default
...
# Loads a video or webcam and finds the pupil in each frame
def process_video(video_path, input_method, camera_index=0):

    # Output path: use current directory so it works on any OS
    out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'output_webcam.mp4')
    ...
    print("Webcam pupil tracking started (center rectangle, 6x zoom). Press Q to quit, D for debug, SPACE to pause.")
    ...
    while True:
        ...
        # Crop and resize: webcam = center rectangle + 10x zoom; video file = aspect-ratio crop
        if input_method == 2:
            frame = crop_roi_and_zoom(frame, zoom=6, width=640, height=480)
        else:
            frame = crop_to_aspect_ratio(frame)
...
# Run with webcam (no file dialog)
def run_webcam(camera_index=0):
    process_video(None, 2, camera_index)
...
def select_video():
    ...
    # second parameter: 1 = video file, 2 = webcam
    process_video(video_path, 1)

if __name__ == "__main__":
    # Run on webcam by default. Use: python pupil_tracker_webcam.py
    # For video file instead, call select_video()
